...
def lines(image):
  blur = cv2.medianBlur(image,5) #filtro para reduccion de ruido
...
def callback(img):
  ...
  gray = cv2.cvtColor(color, cv2.COLOR_BGR2GRAY) #escala de grises
  blur = cv2.medianBlur(gray,5) #filtro para reduccion de ruido

  signal = blur[250:450,1050:]
  cv2.imwrite("imagen.png",signal)
  _,thresh_signal = cv2.threshold(signal,125,255,cv2.THRESH_BINARY)
  image = blur[550:900,60:] #recorte de imagen
...
"""
  ...
  cv2.imshow("Image window", thresh_signal)
  cv2.waitKey(1)
